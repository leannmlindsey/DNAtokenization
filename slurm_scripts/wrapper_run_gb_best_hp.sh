...
# Hyena
#LOG_DIR="../watch_folder/gb_cv10/hyena"
...
#RC_AUGS=( "true" )
#LRS=( "6e-3" "6e-4" "6e-5" )
#CSV_FILE=${CSV_FILE:-"best_hp_hyena_GB.csv"}
...
#RC_AUGS=( "true" )
#LRS=("1e-4" "2e-4" "5e-5")
#CSV_FILE=${CSV_FILE:-"best_hp_mamba_char_GB.csv"}
...
LRS=("2e-3" "1e-3")
LRS=("1e-4" "5e-4" "1e-5")
CSV_FILE=${CSV_FILE:-"hp_round3/best_hp_caduceus_GB.csv"}
...
while IFS=, read -r col1 col2 col3 col4 col5 remainder || [ -n "$col1" ]; do
  ...
  echo "Processing line $line_num: TASK=$TASK, LR=$LR, BATCH_SIZE=$BATCH_SIZE"
    ...
    if [ "$DRY_RUN" = "true" ]; then
      ...
      echo "                          \"run_genomics_benchmark.sh\" "
    else
