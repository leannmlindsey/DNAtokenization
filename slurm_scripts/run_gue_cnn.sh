...
#SBATCH --output=../watch_foldergue__cnn_%j.log  # Log file
...
for seed in $(seq 1 10); do
  HYDRA_RUN_DIR="./outputs/downstream/gue_cv10_ep20/${TASK}/${WANDB_NAME}/seed-${seed}"
  mkdir -p "${HYDRA_RUN_DIR}"
  echo "*****************************************************"
  echo "Running GUE TASK: ${TASK}, lr: ${LR}, batch_size: ${BATCH_SIZE}, RC_AUG: ${RC_AUG}, SEED: ${seed}"
  python -m train \
    ...
    wandb.group="downstream/gue_cv10_ep20" \
    wandb.job_type="${TASK}" \
    wandb.name="${WANDB_NAME}" \
    wandb.id="gue_cv10_ep20_${TASK}_${WANDB_NAME}_seed-${seed}" \
